...
#cur_dir=$(pwd)
#set history
wget -q http://10.199.89.56/wanda/serverinit/history-set.sh -O /etc/profile.d/history-set.sh

#install pkg
yum -y install net-snmp-utils tigervnc-server mysql-server MySQL-python screen lrzsz
...
wget -q http://10.199.89.56/wanda/ntp/ntp.conf.tms -O /etc/ntp.conf
